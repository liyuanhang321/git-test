...
// REQUIRES 声明调用线程必须拥有对指定的 capability 具有独占访问权。可以指定多个
// capabilities。
...
// RELEASE 和 RELEASE_SHARED 与 ACQUIRE 和 ACQUIRE_SHARED
// 正相反，它们表示调用方在调用该函数/方法时需要先持有锁，
...
// ACQUIRE 和 ACQUIRE_SHARED 的尝试版本，第一个参数是 bool，true 代表成功，false
// 代表失败
#define TRY_ACQUIRE(...) \
...
// NO_THREAD_SAFETY_ANALYSIS 表示关闭某个函数/方法的 TSA
// 检测，通常只用于两种情况：
